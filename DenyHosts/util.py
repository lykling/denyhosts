...
def setup_logging(prefs, enable_debug, verbose, daemon):
    if daemon:
        ...
        if daemon_log:
            # define a Handler
            # which writes INFO messages or higher to the sys.stderr
            fh = logging.FileHandler(daemon_log, 'a')
            fh.setLevel(logging.DEBUG)
            msg_fmt = prefs.get('DAEMON_LOG_MESSAGE_FORMAT')
            time_fmt = prefs.get('DAEMON_LOG_TIME_FORMAT')
...
def is_true(pattern):
    ...
    if pattern in ('1', 't', 'true', 'y', 'yes'):
        return True
    else:
        return False


def is_false(pattern):
    return not is_true(pattern)
...
def calculate_seconds(timestr, zero_ok=False):
    ...
    if type(timestr) is IntType:
        return timestr

    m = TIME_SPEC_REGEX.search(timestr)
    if not m:
        except_msg = "Invalid time specification: string format error: %s"
        raise Exception, except_msg, timestr

    units = int(m.group('units'))
    period = m.group('period') or 's'  # seconds is the default

    if units == 0 and not zero_ok:
        except_msg = "Invalid time specification: units = 0"
        raise Exception, except_msg, units
...
""" % (prefs.get('SMTP_FROM'),
    ...
    msg += report_str
    try:
        host = prefs.get('SMTP_HOST')
        port = prefs.get('SMTP_PORT')
        smtp = SMTP(host, port)
        if host == 'smtp.gmail.com' or host == 'mail.google.com':
            smtp.starttls()
